...
def pre(token_list, tokenizer):
    ...
    return res
@torch.no_grad()
def predict(model, data_loader, tokenizer, device, config):
    # test
    model.eval()
    ...
    with open('/data1/ach/project/T5summarization/output/3.0.0_small_eval/{}.json'.format(utils.get_rank()),'w') as f:
...
def main(args, config):
    ...
    train_loader, val_loader, test_loader = create_loader(datasets,samplers,
                                                          batch_size=[config['batch_size_train']]+[1]*2,
    ...
    test_stats = predict(model, test_loader, tokenizer, device, config)
