...
class summarization_dataset(Dataset):
    def __init__(self, ann_file,max_words=30,prefix="",use_prompt=False):        
        ...
        if use_prompt:
            ...
            for temp in self.ann:
                average_rouge_L+=temp["rougeLsum"][1]
            self.average_rouge_L = average_rouge_L/len(self.ann)
        self.average_rouge_L = 0.874
    def __len__(self):
        return len(self.ann)
    ...
    def __getitem__(self, index):    
        ...
        if self.use_prompt:
            ...
            text = ann["text"]+self.prefix+"generate {} abstractive summary: ".format('high' if ann["rougeLsum"][1]< self.average_rouge_L else "low" )
            # pass
        else:
...
class summarization_test_dataset(Dataset):
    def __init__(self, ann_file,max_words=30,prefix="",use_prompt=False):        
        self.ann = json.load(open(ann_file,'r'))
        self.max_words = max_words
        self.prefix = prefix
    ...
    def __getitem__(self, index):    
        ...
        if self.use_prompt:
            # text = self.prefix+"generate {} abstractive summary. ".format('high' if ann["rougeL"][-1]< self.average_rouge_L else "low" )+ann["text"]
            text = self.prefix+"generate {} abstractive summary: ".format('high' if ann["rougeLsum"][1]< self.average_rouge_L else "low" )+ann["text"]
            # pass
        else:
            text = self.prefix+ann["text"]
        # text = pre_caption(text,self.max_words)
